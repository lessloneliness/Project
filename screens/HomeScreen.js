...
const HomeScreen = () => {
  ...
  const user = auth.currentUser; // from auth fire base
    ...
    getDoc(userRef)
      .then(function (doc) {
        if (doc.exists()) {
          ...
          getDownloadURL(ref(storage, `${user.uid}.jpg`))
            .then((url) => {
              url && setImage(url);
            })
            .catch((error) => {
              console.log('an error occured while fetching image');
            });
        } else {
  ...
  const pickImage = async () => {
    ...
    const storage = getStorage();
    const picRef = ref(storage, `${user.uid}.jpg`);
      ...
      <SafeAreaView>
        ...
        <View style={styles.container}>
          ...
          >
            <TouchableOpacity
              onPress={() =>
                navigation.navigate('Add Friend Screen', {
                  bio: bioFromDb,
                  hobbies,
                })
              }
